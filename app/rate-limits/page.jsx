...
import { House, ChatText, Prohibit, Monitor, Key } from "phosphor-react";
import './style.css';
...
export default function Home() {
    ...
    const [rateLimit, setRateLimit] = useState("");  // Estado para almacenar el rate limit ingresado
    const [message, setMessage] = useState(null); // Estado para mostrar mensajes de éxito/error
    const [isSaving, setIsSaving] = useState(false); // Estado para mostrar el estado de carga

    useEffect(() => {
      const fetchRateLimit = async () => {
          try {
              const response = await fetch("http://localhost:5000/logs/get_rate_limit");
    ...
    const handleSaveRateLimit = async () => {
        ...
        try {
            const response = await fetch("http://localhost:5000/logs/update_rate_limit", {
                ...
                body: JSON.stringify({ rate_limit: parseInt(rateLimit) })
            });
